	...
	for i in range(anzahl_iterationen):
		...
		if debug:
			...
			print(
					...
					f"{linalg.norm(A_v, 2)} \n = \n "
					f"{(A_v / linalg.norm(A_v, 2).reshape(-1, 1)).reshape(-1, 1)} (Normiert)",
			)
			print()
			print(
...
def max_eigenwert_toleranz(A, v_0, toleranz, debug = False):
	...
	while np.abs(linalg.norm(v_next - v_previous, ord = 2)) > toleranz:
		...
		if debug:
			...
			print()
			print(
					f"λ_{iteration_count} = ((v_{iteration_count - 1})^T * Av_{iteration_count - 1}) / ((v_"
					f"{iteration_count - 1})^T * v_{iteration_count - 1}): \n ({v_previous.T} \n * \n "
					f"{A_v.reshape(-1, 1)}) \n / \n ({v_previous.T} \n * \n {v_previous.reshape(-1, 1)}) \n = \n "
					f"{eigenwert_näherung}",
			)
			print()
