...
def berechne_eigenwert_mit_charakteristischem_polynom(A):
	...
	try:
		expr = [
				(f"({i + 1}) λ = {lambda_aka_eigenwerte[i]} mit algebraischer Vielfachheit "
				 f"{roots[lambda_aka_eigenwerte[i]]} \n")
...
def geometrische_vielfachheit(R):
	print(
			f"Geometrische Vielfachheit = n - Rg(A - λI) = {R.shape[0]} - {np.linalg.matrix_rank(R)} ="
			f" {R.shape[0] - np.linalg.matrix_rank(R)}",
	)
...
# Wichtig! Eigenraum selber von Eigenvektoren ablesen, parametrisierte Lösung kann momentan nicht weiterverarbeitet
# werden um den Raum zu bestimmen
